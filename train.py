...
reg_params = ['inception4a', 'inception4b', 'inception4c', 'inception4d', 'inception4e', 'inception5a', 'inception5b', 'last_fc']
    ...
    try:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for i, batch in enumerate(dataset):
                ...
                negatives = negatives.view(negatives.size(0), args.att_heads, -1)
                '''
                embeddings, atts = model(x, sampling=False, ret_att=True)
                ...
                writer.add_scalars(main_tag='TrainLog', tag_scalar_dict={'homo': l_homo.item(), 'heter': l_heter.item(), 'div': l_div.item(), 'reg': l_reg.item()},
                                    global_step=step)
                if i % 100 == 0:
                    print('LR:', get_lr(optimizer))
                    print('\tBatch %d\tloss div: %.4f (%.3f)\tloss homo: %.4f (%.3f)\tloss heter: %.4f (%.3f)\tloss_reg: %.4f (%.3f)'%\
                        (i, l_div.item(), loss_div/(i+1), l_homo.item(), loss_homo/(i+1), l_heter.item(), loss_heter/(i+1), l_reg.item(), loss_reg/(i+1)))
                if i % 100 == 0:
            ...
            loss_div /= (i+1)
            loss_reg /= (i+1)
            print('Epoch %d batches %d\tdiv:%.4f\thomo:%.4f\theter:%.4f\treg:%.4f'%(epoch, i+1, loss_div, loss_homo, loss_heter, loss_reg))
            writer.add_scalars(main_tag='Train', tag_scalar_dict={'homo': loss_homo, 'heter': loss_heter, 'div': loss_div, 'reg': loss_reg},
                                global_step=epoch)
